...
if (-not $SkipValidation) {
    ...
    $nuitkaCheck = python -c "import nuitka; print(nuitka.__version__)" 2>&1
    if ($LASTEXITCODE -eq 0 -and $nuitkaCheck -match "\d+\.\d+") {
        Write-Host " OK (v$($nuitkaCheck.Trim()))" -ForegroundColor Green
    } else {
